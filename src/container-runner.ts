...
function buildVolumeMounts(group: RegisteredGroup, isMain: boolean): VolumeMount[] {
  ...
  // Per-group Cursor sessions directory (isolated from other groups)
  // Each group gets their own .cursor/ to prevent cross-group session access
  const groupSessionsDir = path.join(DATA_DIR, 'sessions', group.folder, '.cursor');
  ...
  // Only expose specific auth variables needed by Cursor CLI, not the entire .env
  const envDir = path.join(DATA_DIR, 'env');
  fs.mkdirSync(envDir, { recursive: true });
  const envFile = path.join(projectRoot, '.env');
  if (fs.existsSync(envFile)) {
    const envContent = fs.readFileSync(envFile, 'utf-8');
    const allowedVars = ['CURSOR_API_KEY'];
